...
export default function StreamixModal() {
  ...
  useEffect(() => {
    ...
    const pollJobStatus = async () => {
      try {
        ...
        const job = downloads.find((d) => d.id === activeJobId || d._id === activeJobId || d.jobId === activeJobId);
        if (job) {
  ...
  const handlePause = async (e) => {
    ...
    try {
      const apiBaseUrl = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000/api';
      const res = await fetch(`${apiBaseUrl}/downloads/${activeJobId}/pause`, { method: 'POST' });
      if (res.ok) {
        setStatus('paused');
        setSpeed('Paused');
        setEta('--');
  ...
  const handleResume = async (e) => {
    ...
    try {
      const apiBaseUrl = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000/api';
      const res = await fetch(`${apiBaseUrl}/downloads/${activeJobId}/resume`, { method: 'POST' });
      if (res.ok) {
        setStatus('queued');
        setSpeed('0 B/s');
        setEta('Queued');
  ...
  const handleCancel = async (e) => {
    ...
    try {
      const apiBaseUrl = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000/api';
      const res = await fetch(`${apiBaseUrl}/downloads/${activeJobId}`, { method: 'DELETE' });
      if (res.ok) {
        setActiveJobId(null);
        setIsOpen(false);
  ...
  if (isMinimized && activeJobId && !isConfiguring) {
    return (
      <div className="streamix-minimized-pill" onClick={handlePillClick}>
        <div className={`streamix-minimized-icon-pulse ${status === 'paused' ? 'paused' : ''}`} />
  ...
  return (
    <div className="streamix-modal-overlay" onClick={() => !activeJobId && setIsOpen(false)}>
        ...
        <div className="streamix-modal-header">
          <div style={{ minWidth: 0, flex: 1, marginRight: '12px' }}>
            <h3 className="streamix-modal-title">
              {!isConfiguring && activeJobId ? 'Download Progress' : 'Streamix Download Config'}
            </h3>
            <p className="streamix-modal-subtitle" title={videoTitle}>{videoTitle}</p>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            {isConfiguring && (
              <button type="button" className="streamix-modal-close" onClick={() => setIsOpen(false)}>
          ...
          <div className="streamix-progress-card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '11px', fontWeight: 'bold', color: '#94a3b8' }}>
                {status === 'completed' ? 'Transfer Complete' : 'Media Streaming Progress'}
              </span>
              <span className={`streamix-progress-status-badge streamix-status-${status}`}>
                {status}
              </span>
            </div>
            ...
            <div className="streamix-progress-stats">
              ...
              <span>
                ...
                {eta && eta !== '--' && eta !== 'Queued' ? ` • ETA: ${eta}` : ''}
              </span>
            </div>

            {errorMsg && (
              <div style={{ color: '#f87171', fontSize: '11px', marginTop: '2px', fontWeight: 'bold' }}>
                {errorMsg}
              </div>
            )}
            ...
            <div className="streamix-progress-actions">
              {status === 'completed' || status === 'failed' ? (
                ...
                <>
                  {status === 'paused' ? (
                    <button type="button" className="streamix-btn-progress-action" onClick={(e) => handleResume(e)}>
                      Resume
                    </button>
                  ) : (
                    <button 
          ...
          <>
            ...
            {format === 'video' && (
              <div className="streamix-form-group">
                ...
                <select className="streamix-select" value={quality} onChange={(e) => setQuality(e.target.value)}>
            ...
            >
              {isSubmitting ? 'Starting Download...' : 'Confirm Download Stream'}
            </button>
          </>
        )}
      </div>
